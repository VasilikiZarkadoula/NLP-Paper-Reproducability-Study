...
def sent_encode(tokenizer, sent):
    ...
    if sent == "":
        ...
        if LooseVersion(trans_version) >= LooseVersion("4.0.0"):  # LooseVersion implements Version numbering
...
def get_model(model_type, num_layers, all_layers=None):
    ...
    if hasattr(model, "decoder") and hasattr(model, "encoder"):  # hasattr() method returns true if an object has the
    ...
    if not all_layers:
        if hasattr(model, "n_layers"):  # xlm
            ...
            assert (
                    0 <= num_layers <= len(model.layer)  # if condition returns False, AssertionError is raised:
...
# padding -> in order to make all sequences in a batch fit a given standard length
...
def bert_encode(model, x, attention_mask, all_layers=False):
    # model.eval()
    """
    This function takes in a BERT model, a batch of input sequences x,
    a corresponding attention mask attention_mask, and a boolean flag all_layers.
    It applies the BERT model to the input sequences with the attention mask and returns the output.
    The output_hidden_states flag specifies whether to return all the hidden states of the model,
    or just the final hidden state. If all_layers is True, the function returns a stack of all the hidden states.
     Otherwise, it returns just the final hidden state.
    """
...
def process(a, tokenizer=None):
    if tokenizer is not None:
        a = sent_encode(tokenizer, a)  # Encoding as sentence based on the tokenizer
    return set(a)
...
def collate_idf(arr, tokenizer, idf_dict, device="cuda:0"):
    ...
    """
    arr = [sent_encode(tokenizer, a) for a in arr]  # does the tokenization of the sentences

    idf_weights = [[idf_dict[i] for i in a] for a in arr]  # idf scores for words in the sentences

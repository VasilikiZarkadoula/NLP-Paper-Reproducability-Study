...
from fastNLP import FitlogCallback, LossInForward,RandomSampler, Trainer, WarmupCallback, cache_results ,GradientClipCallback
...
def parse_args():
    ...
    parser.add_argument("--lr", default=5e-4, type=float, required=False)           #learning rate
...
class BERTScore(torch.nn.Module):
    """
    BERTScore is a PyTorch Module which takes in reference sentences refs, candidate sentences hyps, and labels,
    and returns the BERTScore for the input. It initializes a BERT model and an adapter module,
    and sets the inverse document frequency (idf) values for special tokens [SEP] and [CLS] to 0.
    It also has functions to save the adapter module and to forward the input through the model to compute the BERTScore.
    """
...
if __name__ == '__main__':
    ...
    trainer = Trainer(train_data=train_data, model=model, loss=LossInForward(), optimizer=optimizer,
                      ...
                      num_workers=5, n_epochs=args.n_epochs, print_every=50, dev_data=None, metrics=None,
